...
class BitStream {
    out = [];
    buf = 0; // accumulated bits (not yet flushed to out)
    fill = 0; // number of valid bits in buf
    write(code, len) {
        ...
        while (this.fill >= 8) {
            ...
            this.out.push(b);
            if (b === 0xff)
                this.out.push(0x00); // JPEG byte stuffing
        }
    }
    flush() {
...
function ampCode(n, bits) {
    return n >= 0 ? n : n + (1 << bits) - 1;
}
// ── Encode a single 8×8 DCT block ─────────────────────────────────────────────
function encodeBlock(bs, dct, // 64 DCT coefficients in natural 8×8 order
quant, // 64 quantization values in natural 8×8 order
dcTab, acTab, prevDC) {
    ...
    if (dcBits > 0)
        bs.write(ampCode(dc, dcBits), dcBits);
...
function writeDQT(arr, tableId, table) {
    arr.push(0xff, 0xdb); // DQT marker
    u16be(arr, 67); // Length: 2 + 1 + 64
    arr.push(tableId); // Precision(0=8-bit) | table id
    for (let i = 0; i < 64; i++)
        arr.push(table[ZZ[i]]); // zigzag order
}
function writeDHT(arr, tc, th, bits, vals) {
 ...
 */
export function encodeJpeg(width, height, pixels, quality = 85) {
    ...
    u16be(hdr, 1);
    u16be(hdr, 1); // pixel aspect ratio 1:1
